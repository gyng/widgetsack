; WIDGETSACK FORK of Tauri's bundled NSIS installer template (tauri-bundler 2.9.2's installer.nsi),
...
; src/bundle/windows/nsis/installer.nsi and re-apply the WIDGETSACK additions.
Unicode true
...
; checkbox with ${NSD_*} (DPI-correct, same x as the built-in Run / desktop-shortcut checkboxes).
...
Function WS_AutostartShow
  ...
  ; flag) → checked. On a reinstall we honour the prior choice: checked when pref=1, and only left
  ; unchecked when the user explicitly opted out before (pref=0).
  ClearErrors
  ReadRegDWORD $0 HKCU "Software\io.github.gyng\widgetsack" "LaunchAtLogin"
  ${If} ${Errors}
  ${OrIf} $0 = 1
    ${NSD_Check} $WS_AutostartCheckbox
  ${EndIf}
FunctionEnd

Function WS_AutostartLeave
  ${NSD_GetState} $WS_AutostartCheckbox $0
  ${If} $0 = ${BST_CHECKED}
    WriteRegDWORD HKCU "Software\io.github.gyng\widgetsack" "LaunchAtLogin" 1
  ${Else}
    WriteRegDWORD HKCU "Software\io.github.gyng\widgetsack" "LaunchAtLogin" 0
  ${EndIf}
FunctionEnd
...
Section Uninstall
  ...
  ${AndIf} $UpdateMode <> 1
    ...
    ; WIDGETSACK: remove the launch-at-login preference too. Guarded by the same condition as the
    ; rest of the data wipe (user explicitly ticked "delete app data" AND not an update), so it
    ; survives ordinary upgrades — only an intentional uninstall-with-data clears it.
    DeleteRegValue HKCU "Software\io.github.gyng\widgetsack" "LaunchAtLogin"
    DeleteRegKey /ifempty HKCU "Software\io.github.gyng\widgetsack"
  ${EndIf}
