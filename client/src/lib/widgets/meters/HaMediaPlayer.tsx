// Interactive HA meter (molecule): a media_player — album art + now-playing text + transport (prev /
// play-pause / next) + a volume slider with mute. Reads title/artist/state/volume from the entity
// JSON (binds: 'json'); controls emit onControl (media_player.media_play_pause / media_next_track /
// media_previous_track / volume_set / volume_mute) which Canvas turns into ha_call_service. volume
// service_data is built by the pure core/haControls helper. Prop-only (AGENTS.md §6) — the `art` URL
...
type Props = {
	...
	showVolume?: boolean;
	art?: string; // resolved cover URL (art-scheme), supplied by the host
};
...
export default function HaMediaPlayer({
	...
	return (
		<div className="ha-media np-ha-media" data-part="root">
			{art ? <img className="art" data-part="art" src={art} alt="" /> : null}
			<div className="np-ha-media-body">
